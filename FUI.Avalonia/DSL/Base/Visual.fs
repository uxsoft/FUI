...
type VisualBuilder<'t when 't :> Visual and 't : equality>() =
    ...
    member _.renderTransform<'t, 'v>(x: Types.AvaloniaNode<'t>, transform: 'v) =
    ...
    member _.renderTransformOrigin<'t, 'v>(x: Types.AvaloniaNode<'t>, origin: 'v) =
        Types.dependencyProperty x Visual.RenderTransformOriginProperty origin
        
    /// int | ObservableValue<int>
    [<CustomOperation("zIndex")>]
    member _.zIndex<'t, 'v>(x: Types.AvaloniaNode<'t>, index: 'v) =
