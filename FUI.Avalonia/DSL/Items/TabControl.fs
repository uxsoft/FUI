...
type TabControlBuilder<'t when 't :> TabControl and 't : equality>() =
    ...
    member _.contentTemplate<'t, 'v>(x: Types.AvaloniaNode<'t>, value: 'v) =
    ...
    member _.selectedContent<'t, 'v>(x: Types.AvaloniaNode<'t>, value: 'v) =
        Types.dependencyProperty x TabControl.ContentTemplateProperty value
        
    [<CustomOperation("onSelectedContentChanged")>]
    member _.onSelectedContentChanged<'t, 'v>(x: Types.AvaloniaNode<'t>, func: obj -> unit) =
        Types.dependencyPropertyEvent x TabControl.SelectedContentProperty func
        
    /// IDataTemplate | ObservableValue<IDataTemplate>
    [<CustomOperation("selectedContentTemplate")>]
    member _.selectedContentTemplate<'t, 'v>(x: Types.AvaloniaNode<'t>, value: 'v) =
    ...
    member _.onSelectedContentTemplateChanged<'t, 'v>(x: Types.AvaloniaNode<'t>, func: IDataTemplate -> unit) =
